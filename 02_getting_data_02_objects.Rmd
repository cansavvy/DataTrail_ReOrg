...
`as.factor()` or `as.numeric()` or `as._____()` etc - Can coerce a existing vector to be of a particular class. So for example, `as.factor()` would try to make the vector a factor. 
`class()`
